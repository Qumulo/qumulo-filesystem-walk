...
class CopyDirectory:
    def __init__(self, args):
        ...
        parser.add_argument('--skip_hardlinks', help=' skip hard links', action='store_true')
        parser.add_argument('--no_preserve', help='will not preserve permissions or timestamps', action='store_true')
        ...
        for level in range(2, len(levels)+1):
            ...
            if new_dir not in self.folders:
                try:
                    ...
                    e_str = "get_file_attr exception creating directory: %s %s" % (sys.exc_info(), traceback.format_exc().replace("\n", ""))
                ...
                try:
                    ...
                    log_it("create_directory exception: %s %s/%s" % (sys.exc_info()[0], dir_path, dir_name))
                    new_f = rc.fs.get_file_attr(path = new_dir)
                    self.folders[new_dir] = new_f['id']
                ...
                if file_obj['type'] == "FS_FILE_TYPE_DIRECTORY":
                    ...
                    try:
                        ...
                        if new_f['size'] == file_obj['size'] and \
                                new_f['modification_time'] == file_obj['modification_time'] and \
                                new_f['change_time'] == file_obj['change_time']:
                            log_it("File exists with same size and timestamp: %s" % to_path)
                            results.append("FILE EXISTS: %s -> %s" % (file_obj['path'], to_path))
                            file_exists = new_f['id']
                        else:
                            print("%s != %s" % (new_f['modification_time'], file_obj['modification_time']))
                            print("%s != %s" % (new_f['change_time'], file_obj['change_time']))
                            work_obj.rc.fs.delete(id_ = new_f['id'])
                    except:
                        if 'fs_no_such_entry_error' not in str(sys.exc_info()[1]):
                            e_str = "get_file_attr exception: %s %s" % (sys.exc_info(), traceback.format_exc().replace("\n", ""))
                            log_it(e_str)
                            results.append("!!FILE COPY FAILED1: %s -> %s" % (file_obj['path'], to_path))
                        pass
                    try:
                        if not file_exists:
                            if file_obj['type'] == 'FS_FILE_TYPE_SYMLINK':
                                ...
                                new_f = work_obj.rc.fs.create_file(dir_path=parent_path, name=file_name)
                        ...
                        o_attr = work_obj.rc.fs.get_file_attr(snapshot = work_obj.snap, id_ = file_obj["id"])
                        o_acl = work_obj.rc.fs.get_acl_v2(snapshot = work_obj.snap, id_ = file_obj["id"])

                        try:
                            for stream in work_obj.rc.fs.list_named_streams(snapshot=work_obj.snap, id_=file_obj["id"]):
                                log_it("Create stream: %s - %s" % (file_exists, stream['name']))
                                new_st = work_obj.rc.fs.create_stream(id_=file_exists, stream_name=stream['name'])
                                log_it("Copy stream: %s:%s:%s -> %s:%s" % (work_obj.snap, file_obj["id"], stream['id'], file_exists, new_st['id']))
                                work_obj.rc.fs.copy(source_snapshot=work_obj.snap,
                            ...
                            e_str = "Stream create/copy exception: %s %s" % (sys.exc_info(), traceback.format_exc().replace("\n", ""))
                            if 'fs_invalid_file_type_error' not in e_str and 'fs_entry_exists_error' not in e_str:
                                log_it(e_str)
                        ...
                        log_it("File create/copy exception: %s %s" % (sys.exc_info(), traceback.format_exc()))
                        results.append("!!FILE COPY FAILED2: %s -> %s" % (file_obj['path'], to_path))
            except:
                log_it("Other exception: %s %s" % (sys.exc_info(), file_obj['path']))
                results.append("!!FILE COPY FAILED3: %s -> %s" % (sys.exc_info(), file_obj['path']))
