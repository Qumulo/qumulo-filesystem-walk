...
class CopyDirectory:
    ...
    def every_batch(file_list, work_obj):
                ...
                if file_obj['type'] == "FS_FILE_TYPE_DIRECTORY":
                    ...
                    try:
                        ...
                        if 'fs_no_such_entry_error' not in str(sys.exc_info()[1]):
                            print("get_file_attr exception: %s %s" % (sys.exc_info(), file_obj['path']))
                    ...
                    try:
                        ...
                        # if 'fs_entry_exists_error' not in str(sys.exc_info()[1]):
                        #     print("File create/copy exception: %s %s" % (sys.exc_info(), file_obj['path']))
                        results.append("!!FILE COPY FAILED2: %s -> %s" % (file_obj['path'], to_path))
            except:
                print("Other exception: %s %s" % (sys.exc_info(), file_obj['path']))
        ...
        try:
            ...
            print("Unable to save results exception: %s" % str(sys.exc_info()))
